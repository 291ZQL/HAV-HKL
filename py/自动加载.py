...
class Spider(BaseSpider):
    ...
    def _scan_base_preview(self, value):
        path = self._normalize_scan_base_path(value) or self.LOCAL_BASE_DIR
        roots = self._scan_roots_for_base(path)
        return "\n".join(
            "{}: {}".format(item["type"], item["path"]) for item in roots
        )
    ...
    def _notify_app(self, message, wait=False, replace=False):
        ...
        try:
            ...
            application = python_class.getPlatform().getApplication()
            try:
                activity = self._current_android_activity(jclass)
            except Exception:
                activity = None
            context = activity if activity is not None else application
    ...
    def _test_sites_locked(self, force=False):
        ...
        try:
            ...
            self._log("ERROR", "站点检测批次失败: {}".format(exc))
    ...
    def _source_probe_path(self, source):
        ...
        if isinstance(site, dict):
            field = "api" if source.get("type") == "JS" else "ext"
    ...
    def _identity_source_path(self, identity):
        ...
        return self._reference_path(parts[1]) if len(parts) == 2 else ""
    ...
    def _dex_defined_spider_classes(self, data):
        ...
        for index in range(class_defs_size):
            ...
            if descriptor.startswith(prefix) and descriptor.endswith(b";"):
                class_name = descriptor[len(prefix) : -1].decode("ascii")
    ...
    def _validate_csp_jar(self, api, jar_reference):
        return self._validate_site_jar(jar_reference, api)
        ...
        if isinstance(ext, str) and ext.strip():
            ...
            if ext_path:
                ...
                if ext_path.lower().endswith(".json"):
                    with open(ext_path, "r", encoding="utf-8") as fp:
                        json.load(fp)
                if source_type == "XBPQ":
    ...
    def _parse_csp_manifest(self, path):
        return self._parse_site_manifest(path, "CSP")
            ...
            generated_items.append(
                {
                    "id": source["key"],
                    "enabled": True,
                    "kind": "webHome" if source["type"] == "HTML" else "csp",
                    "site": site,
                }
            )
    ...
    def _status_detail(self):
        ...
        for result in self.site_test_results.values():
            ...
            with self.lock:
                ...
                try:
                    ...
                    message = (
                        ...
                        "重新检测" if summary["retest"] else "连通性检测",
                        summary["tested"],
                        summary["cached"],
                        summary["available"],
                        summary["unavailable"],
                    ...
                    if summary["remaining"]:
                        message += "；剩余 {} 个将在下次点击时检测".format(
                            summary["remaining"]
                        )
                    self.inited = True
                    return {"code": 0, "msg": message}
                except Exception as exc:
                    self._log("ERROR", "站点检测操作失败: {}".format(exc))
                    return {"code": 0, "msg": "站点检测失败：{}".format(exc)}
        ...
        if action == self.ACTION_RESTORE_BACKUP:
            with self.lock:
                try:
                    ...
                    return {
                        "code": 0,
                        "msg": "已恢复上次注册表（{} 个条目）；{}".format(
                            count,
                            detail,
                        ),
                    }
                except Exception as exc:
                    self._log("ERROR", "恢复上次注册表失败: {}".format(exc))
